...
if (isset($_GET['post_id']) && isset($_SESSION['id'])) {
    ...
    $updateQuery = "UPDATE posts SET status = 0 WHERE id = ?";
    $stmt = $dbconn->prepare($updateQuery);
    $stmt->bind_param("i", $post_id);
    $stmt->execute();
    
    // Logolás a post_admin_logs táblába
    $logQuery = "INSERT INTO post_admin_logs (admin_user_id, post_id, action, created_at) VALUES (?, ?, ?, NOW())";
    $stmt = $dbconn->prepare($logQuery);
    $action = 1; // Archiválás
    $stmt->bind_param("iii", $user_id, $post_id, $action);
    $stmt->execute();
        } else {
